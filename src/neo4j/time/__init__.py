...
DATE_ISO_PATTERN = _re_compile(r"^(\d{4})-(\d{2})-(\d{2})$")
TIME_ISO_PATTERN = _re_compile(
    r"^(\d{2})(:(\d{2})(:((\d{2})"
    r"(\.\d*)?))?)?(([+-])(\d{2}):(\d{2})(:((\d{2})(\.\d*)?))?)?$"
)
DURATION_ISO_PATTERN = _re_compile(
    r"^P((\d+)Y)?((\d+)M)?((\d+)D)?"
    r"(T((\d+)H)?((\d+)M)?(((\d+)(\.\d+)?)?S)?)?$"
)
...
FORMAT_F_REPLACE = _re_compile(r"(?<!%)%f")
...
class Clock:
    ...
    def local_offset(cls):
        ...
        # Windows).
        return ClockTime(-int(_mktime(_gmtime(172800))) + 172800)
...
if _t.TYPE_CHECKING:
    ...
    duration_base_class = _timedelta
else:
    duration_base_class = object


class Duration(_t.Tuple[int, int, int, int],  # type: ignore[misc]
               duration_base_class):
    ...
    def __new__(
        ...
        ns = (int(3600000000000 * hours) +
              ...
              int(nanoseconds))
        s, ns = _symmetric_divmod(ns, NANO_SECONDS)
    ...
    def __add__(  # type: ignore[override]
        self, other: _t.Union[Duration, _timedelta]
    ) -> Duration:
    ...
    def __sub__(self, other: _t.Union[Duration, _timedelta]) -> Duration:
    ...
    @property
    def years_months_days(self) -> _t.Tuple[int, int, int]:
    ...
    @property
    def hours_minutes_seconds_nanoseconds(self) -> _t.Tuple[int, int, int, int]:
...
if _t.TYPE_CHECKING:
    ...
    # https://github.com/python/typeshed/issues/8409#issuecomment-1197704527
    date_base_class = _date
else:
    date_base_class = object


class Date(date_base_class, metaclass=_DateType):
    ...
    @classmethod
    def today(cls, tz: _t.Optional[_tzinfo] = None) -> Date:
        ...
        if tz is None:
            ...
            return (
                DateTime.utc_now()
                .replace(tzinfo=_timezone.utc).astimezone(tz)
                .date()
            )
    ...
    @classmethod
    def from_timestamp(
        cls, timestamp: float, tz: _t.Optional[_tzinfo] = None
    ) -> Date:
    ...
    def from_clock_time(
        cls,
        clock_time: _t.Union[ClockTime, _t.Tuple[float, int]],
        epoch: DateTime
    ) -> Date:
    ...
    @property
    def year_month_day(self) -> _t.Tuple[int, int, int]:
        """3-tuple of (year, month, day) describing the date.
        """
        return self.year, self.month, self.day

    @property
    def year_week_day(self) -> _t.Tuple[int, int, int]:
    ...
    @property
    def year_day(self) -> _t.Tuple[int, int]:
    ...
    def __lt__(self, other: _t.Union[Date, _date]) -> bool:
        """``<`` comparison with :class:`.Date` or :class:`datetime.date`."""
        if isinstance(other, (Date, _date)):
            return self.toordinal() < other.toordinal()
        raise TypeError("'<' not supported between instances of 'Date' and %r" % type(other).__name__)

    def __le__(self, other: _t.Union[Date, _date]) -> bool:
        """``<=`` comparison with :class:`.Date` or :class:`datetime.date`."""
        if isinstance(other, (Date, _date)):
            return self.toordinal() <= other.toordinal()
        raise TypeError("'<=' not supported between instances of 'Date' and %r" % type(other).__name__)

    def __ge__(self, other: _t.Union[Date, _date]) -> bool:
        """``>=`` comparison with :class:`.Date` or :class:`datetime.date`."""
        if isinstance(other, (Date, _date)):
            return self.toordinal() >= other.toordinal()
        raise TypeError("'>=' not supported between instances of 'Date' and %r" % type(other).__name__)

    def __gt__(self, other: _t.Union[Date, _date]) -> bool:
        """``>`` comparison with :class:`.Date` or :class:`datetime.date`."""
        if isinstance(other, (Date, _date)):
            return self.toordinal() > other.toordinal()
        raise TypeError("'>' not supported between instances of 'Date' and %r" % type(other).__name__)
    ...
    def __sub__(self, other: _t.Union[Date, _date]) -> Duration:
        ...
    ...
    def time_tuple(self) -> _struct_time:
        ...
        return _struct_time((self.year, self.month, self.day, 0, 0, 0, day_of_week - 1, day_of_year, -1))
    ...
    def to_clock_time(self, epoch: _t.Union[Date, DateTime]) -> ClockTime:
    ...
    def iso_calendar(self) -> _t.Tuple[int, int, int]:
...
if _t.TYPE_CHECKING:
    ...
    # https://github.com/python/typeshed/issues/8409#issuecomment-1197704527
    time_base_class = _time
else:
    time_base_class = object


class Time(time_base_class, metaclass=_TimeType):
    """Time of day.
    ...
    def __init__(
        ...
        second: int = 0,
        nanosecond: int = 0,
        tzinfo: _t.Optional[_tzinfo] = None
    ) -> None:
    ...
    @classmethod
    def now(cls, tz: _t.Optional[_tzinfo] = None) -> Time:
    ...
    @classmethod
    def from_ticks(cls, ticks: int, tz: _t.Optional[_tzinfo] = None) -> Time:
    ...
    __tzinfo: _t.Optional[_tzinfo] = None
    ...
    def _get_both_normalized_ticks(self, other: object, strict=True):
        if (isinstance(other, (Time, _time))
        ...
        utc_offset: _t.Optional[_timedelta] = other.utcoffset()
    ...
    def __lt__(self, other: _t.Union[Time, _time]) -> bool:
        """`<` comparison with :class:`.Time` or :class:`datetime.time`."""
        self_ticks, other_ticks = self._get_both_normalized_ticks(other)
        if self_ticks is None:
            return NotImplemented
    ...
    def __le__(self, other: _t.Union[Time, _time]) -> bool:
        """`<=` comparison with :class:`.Time` or :class:`datetime.time`."""
        self_ticks, other_ticks = self._get_both_normalized_ticks(other)
        if self_ticks is None:
            return NotImplemented
    ...
    def __ge__(self, other: _t.Union[Time, _time]) -> bool:
        """`>=` comparison with :class:`.Time` or :class:`datetime.time`."""
        self_ticks, other_ticks = self._get_both_normalized_ticks(other)
        if self_ticks is None:
            return NotImplemented
    ...
    def __gt__(self, other: _t.Union[Time, _time]) -> bool:
        """`>` comparison with :class:`.Time` or :class:`datetime.time`."""
        self_ticks, other_ticks = self._get_both_normalized_ticks(other)
        if self_ticks is None:
            return NotImplemented
    ...
    def utc_offset(self) -> _t.Optional[_timedelta]:
    ...
    def dst(self) -> _t.Optional[_timedelta]:
    ...
    def tzname(self) -> _t.Optional[str]:
...
if _t.TYPE_CHECKING:
    ...
    # https://github.com/python/typeshed/issues/8409#issuecomment-1197704527
    date_time_base_class = _datetime
else:
    date_time_base_class = object


@_total_ordering
class DateTime(date_time_base_class, metaclass=_DateTimeType):
    ...
    @classmethod
    def now(cls, tz: _t.Optional[_tzinfo] = None) -> DateTime:
    ...
    if _t.TYPE_CHECKING:
        ...
        # alias of now
        @classmethod
        def today(cls, tz: _t.Optional[_tzinfo] = None) -> DateTime:
            ...
    ...
    def __le__(  # type: ignore[override]
        self, other: _t.Union[DateTime, _datetime]
    ) -> bool:
        """
        ``<=`` comparison with :class:`.DateTime` or :class:`datetime.datetime`.
        """
    ...
    def __add__(self, other: _t.Union[Duration, _timedelta]) -> DateTime:
        ...
        if isinstance(other, _timedelta):
            ...
            time_ = Time.from_ticks(_round_half_to_even(
    ...
    @_t.overload
    def __sub__(self, other: _t.Union[Duration, _timedelta]) -> DateTime:
        ...
    ...
    def __sub__(self, other):
        ...
        if isinstance(other, _datetime):
            ...
            return _timedelta(days=days, seconds=t.seconds,
