...
from ...._codec.hydration.bolt import v3 as hydration_v3
...
class Bolt6x0(Bolt):
    ...
    def _make_qid_tracking_handler(self, wrapped_handler=None):
        def handler(metadata):
            qid = metadata.get("qid")
            if isinstance(qid, int) and qid >= 0:
                self.most_recent_qid = qid
            else:
