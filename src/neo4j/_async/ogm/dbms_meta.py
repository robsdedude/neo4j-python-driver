...
class AsyncDBMSMeta:
    ...
    @classmethod
    async def load(cls, driver: AsyncDriver, *, retry: bool) -> te.Self:
        if retry:
            ...
            async with driver.session(
                ...
                result = await session.run(VERSION_QUERY)
...
class AsyncDBMSMetaProvider:
    _current: t.Optional[AsyncDBMSMeta] = None
    _refresh_lock: AsyncLock
    _retry_lock: AsyncCooperativeLock
    _fail_delay: t.Generator[float, None, None]

    def __init__(self, driver: AsyncDriver) -> None:
        self._driver = driver
        self._refresh_lock = AsyncLock()
        self._retry_lock = AsyncCooperativeLock()
        self._reset_retry()

    def _reset_retry(self) -> None:
        self._fail_delay = retry_delay_generator(1, 2, 0.2)

    async def get(self) -> AsyncDBMSMeta:
        ...
        async with self._refresh_lock:
            ...
            if self._current is None:
                self._current = await AsyncDBMSMeta.load(
