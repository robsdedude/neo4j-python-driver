...
from neo4j._codec.packstream._common import (
    PackableBuffer,
    UnpackableBuffer,
)
...
@pytest.fixture
def unpack(unpacker_with_buffer):
    unpacker, unpackable_buffer = unpacker_with_buffer

    def _unpack(data, hydration_hooks=None):
        unpackable_buffer.data = bytearray(data)
        unpackable_buffer.used = len(data)
...
class TestPackStreamV1:
    ...
    def test_illegal_types(self, value, pack):
        with pytest.raises(ValueError) as exc:
            pack(value)

        msg = str(exc.value)
        assert str(type(value)) in msg
        assert "bolt" not in msg.lower()
    ...
    def test_unpacking_undefined_marker(self, marker, unpack):
        data = marker + (b"\xc0" * 128)
        with pytest.raises(ValueError) as exc:
            unpack(data)

        int_marker = int.from_bytes(marker, "big")
        msg = str(exc.value)
        assert re.search(r"\bmarker\b", msg, re.IGNORECASE)
        assert re.search(rf"\b{int_marker:02x}\b", msg, re.IGNORECASE)
