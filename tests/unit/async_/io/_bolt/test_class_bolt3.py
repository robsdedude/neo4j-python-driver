...
from neo4j._async.io._bolt._bolt3 import AsyncBolt3
...
def test_db_extra_not_supported_in_begin(fake_socket):
    address = neo4j.Address(("127.0.0.1", 7687))
    connection = AsyncBolt3(
        address, fake_socket(address), AsyncPoolConfig.max_connection_lifetime
    )
...
def raises_if_db(db):
    if db is None:
        ...
        with pytest.raises(
            ConfigurationError, match="selecting database is not supported"
        ):
            yield
