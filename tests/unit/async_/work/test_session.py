...
@mark_async_test
async def test_session_run_idempotent_retry(
    script1_type: str,
    ...
    connection_1 = async_scripted_connection_generator()
    connection_1.set_script(script1)
    connection_1.enable_telemetry_matching()
    connection_2 = async_scripted_connection_generator()
    connection_2.set_script(script2)
    connection_2.enable_telemetry_matching()

    async_fake_pool.buffered_connection_mocks.append(connection_2)
    async_fake_pool.buffered_connection_mocks.append(connection_1)
    ...
    async with AsyncSession(async_fake_pool, session_config) as session:
        if should_succeed:
            await session.run("RETURN 1")
        else:
        ...
        if should_use_connection2:
            assert len(async_fake_pool.acquired_connection_mocks) == 2
