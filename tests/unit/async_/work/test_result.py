...
async def test_result_iteration_mixed_methods():
    ...
    record = await anext(iter1)
    assert record.get("x") == records[0][0]
    record = await anext(iter2)
...
async def test_result_peek(records, fetch_size):
    ...
    for i in range(len(records) + 1):
        ...
        if i == len(records):
            ...
            iter_ = aiter(result)
            await anext(iter_)  # consume the record
...
@pytest.mark.parametrize(
    "async_cb",
    (True, False) if AsyncUtil.is_async_code else (False,),
)
