...
from neo4j._sync.addressing import _MultiAddress
...
POOL_ADDRESS = _MultiAddress("neo4j", (ROUTER1_ADDRESS,))


def _simple_pool(opener) -> Neo4jPool:
    return Neo4jPool(
        opener, _pool_config(), WorkspaceConfig(), POOL_ADDRESS
    )
...
@mark_sync_test
def test_acquires_new_routing_table_if_deleted(
    ...
    cx = pool.acquire(READ_ACCESS, 30, db, None, None, None)
    pool.release(cx)
...
@pytest.mark.parametrize("guessed_db", (True, False))
@mark_sync_test
def test_db_resolution_callback(custom_routing_opener, guessed_db):
    cb_calls = []
...
def test_reuses_connection(opener):
    ...
    pool.release(cx1)
    cx2 = pool.acquire(READ_ACCESS, 30, TEST_DB1, None, None, None)
    assert cx1 is cx2
...
def test_does_not_close_stale_connections_in_use(opener):
    ...
    cx3 = pool.acquire(READ_ACCESS, 30, TEST_DB1, None, None, None)
    pool.release(cx3)
    cx1.close.assert_called_once()
    assert cx2 is cx3
...
def test_multiple_broken_connections_on_close(opener, mocker):
    def mock_connection_breaks_on_close(cx):
        def close_side_effect():
            ...
            pool.deactivate(READER1_ADDRESS)

        cx.attach_mock(
            mocker.MagicMock(side_effect=close_side_effect), "close"
        )
...
def test__acquire_new_later_with_room(opener):
    ...
    pool = Neo4jPool(opener, config, WorkspaceConfig(), POOL_ADDRESS)
    assert pool.connections_reservations[READER1_ADDRESS] == 0
    creator = pool._acquire_new_later(
        READER1_ADDRESS, READER1_ADDRESS, None, Deadline(1)
    )
...
@mark_sync_test
def test_discovery_is_retried(custom_routing_opener, error):
...
@mark_sync_test
def test_fast_failing_discovery(custom_routing_opener, error):
    opener = custom_routing_opener(
        [
            None,  # first call to router for seeding the RT with more routers
            error,  # will be retried
...
def test_connection_error_callback(
    ...
    cxs_write = [
        pool.acquire(WRITE_ACCESS, 30, TEST_DB1, None, None, None)
        for _ in range(5)
    ]
...
@mark_sync_test
def test_pool_closes_connections_dropped_from_rt(custom_routing_opener):
...
@mark_sync_test
def test_pool_does_not_close_connections_dropped_from_rt_for_other_server(  # noqa: E501
    custom_routing_opener,
):
    readers = {
    ...
    cx2 = pool.acquire(READ_ACCESS, 30, TEST_DB2, None, None, None)
    pool.release(cx2)
    assert cx2.unresolved_address == READER1_ADDRESS
    cx1.close.assert_not_called()
    ...
    cx3 = pool.acquire(READ_ACCESS, 30, TEST_DB2, None, None, None)
    pool.release(cx3)
...
@mark_sync_test
def test_tracks_ssr_connection_hints(custom_routing_opener):
    connection_count = 0
