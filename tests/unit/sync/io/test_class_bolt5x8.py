...
def test_extra_in_begin(fake_socket, args, kwargs, expected_fields):
    address = neo4j.Address(("127.0.0.1", 7687))
    socket = fake_socket(address, Bolt5x8.UNPACKER_CLS)
    connection = Bolt5x8(
...
def test_tx_timeout(
    ...
    sockets.server.send_message(b"\x70", {})
    connection = Bolt5x8(address, sockets.client, 0)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (None, Warning),
        (1.2, Warning),
        ("example.com", neo4j.Address(("example.com", 7687))),
        ("example.com:1234", neo4j.Address(("example.com", 1234))),
    ),
)
@mark_sync_test
def test_address_callback(
    advertised_address, expected_call, fake_socket_pair, caplog
):
    cb_calls = []

    def cb(connection_, address_):
    ...
    success_meta = {}
    if advertised_address is not ...:
        success_meta["advertised_address"] = advertised_address
    sockets.server.send_message(b"\x70", success_meta)

    connection = Bolt5x8(address, sockets.client, 0, address_callback=cb)

    connection.logon()
    connection.send_all()

    if type(expected_call) is type and issubclass(expected_call, Warning):
        with caplog.at_level(logging.WARNING):
            connection.fetch_all()
        warning_logs = [rec.message for rec in caplog.records]
        assert len(warning_logs) == 1
        assert "NON-FATAL PROTOCOL VIOLATION" in warning_logs[0]
        assert not cb_calls
        return
